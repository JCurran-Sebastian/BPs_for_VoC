...
betavec = Re * gam * ones(1,1); % Transmissibility is identical in all types
...
outputMat = NaN(Ns,3); % Matrix of output with structure:
...
at0 = false(Ns,lcc);
episample = zeros(Ns,3,lcc);
stillgoing2 = false(Ns,lcc);
at02 = false(Ns,lcc);
episample2 = zeros(Ns,3,lcc);
cctimes = [cctimes,Inf]; % To allow extra index out of loop

Ne = 100000; % Number of events (initial guess - could be improved)
Tidx = 1; Eidx = 2; Iidx = 3; % Indices of time columns, E columns (8 TaB), and I columns (8 TaB)
introrate = NaN(Ns,1);
...
for is = 1:Ns
    EL = NaN(Ne,3); % Time, ES1-4, ER1-4, IS1-4, IR1-4
...
%         end
    ...
    episample2(is,:,1) = EL(1,:);
    stillgoing2(is,1) = true;
    for icc = 2:lcc
        stepover = find(EL(:,1)>cctimes(icc),1);
        if ~isempty(stepover)
            episample2(is,:,icc) = EL(stepover-1,:);
            stillgoing2(is,icc) = true; % epidemic still going by this time
            at02(is,icc) = ~any(EL(stepover-1,2:end)); % epidemic at this time is zero
        end      
    end
...
%     save('50k_run.mat');
% end
save(['SEIR_imm02_Re18',version,'.mat'])
%%
% load('50k_run.mat');
load(['SEIR_imm02_Re18',version,'.mat'])
...
%%
figure(3); histogram(timelastevent)

% Cross-check with analytical result
% newordertab = [ 1 ];
% neworder = [ newordertab, 1 + newordertab ];
ccstillgoing = (sum(stillgoing)/Ns)';
ccat0 = (sum(at0)/Ns)';
stillgoingandnon0 = stillgoing & ~at0;
ccstillgoingnon0 = (sum(stillgoingandnon0)/Ns)';
ccmeanbytype = zeros(2,lcc);
for icc = 1:lcc
%     ccmeanbytype(:,icc) = mean(episample(stillgoingandnon0(:,icc),:,icc))';
    ccmeanbytype(:,icc) = mean(episample(:,2:3,icc))';
end
totalsample = permute(sum(episample,2),[1,3,2]);
ccmean = mean(totalsample)';
ccstd = std(totalsample)';
ccvar = var(totalsample)';
csvwrite('SEIR_mean_10days_sim.csv',ccmeanbytype);
csvwrite('SEIR_rho_10days_sim.csv',ccat0);
% csvwrite('SEIR_sd_10days_sim.csv',ccstd);
% csvwrite('SEIR_var_10days_sim.csv',ccvar);

comp = episample - episample2;
comp2 = stillgoing - stillgoing2;
comp3 = at0 - at02;